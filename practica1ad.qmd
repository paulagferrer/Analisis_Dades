...
```{r, message=FALSE}
ruta<- "C:/Users/marce/Downloads/datos_empresa.csv"
...
En primer lugar, tenemos que obtener los datos cuantitativos solamente.

```{r}
datos_cuant <- datos_empresa %>% select(where(is.numeric)) %>% glimpse()
```
...
medias <- datos_cuant %>% as.matrix() %>% {1/n * (v1 %*% .)}

H <- diag(n) - 1/n * (v1 %*% t(v1))

covarianzas <- datos_cuant %>% as.matrix() %>% {1/n * (t(.) %*% H %*% .)}
```


Hasta aquí de forma matricial. Ahora vayamos a calcularlo con funciones de R/Tidyverse:

```{r}
medias2 <- datos_cuant %>% summarise(across(everything(), mean))
covarianzas2 <- datos_cuant %>% as.matrix() %>% cov() %>% {. * (n-1)/n}
```
...
```{r}
correlaciones <- datos_cuant %>% as.matrix() %>% cor()

ggpairs(correlaciones)
```
