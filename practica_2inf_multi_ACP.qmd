...
nota: errores que me he dado cuenta

Fila 1 de la matriz corresponde al primer código del fichero de phylums (`hsa`),fila 2 corresponde al segundo (`ptr`) … y así hasta 369. Esto tirne sentido ya que las matrices tienen 369 fila, el fichero de phylum tiene 369 líneas. NO hay que usar row.names (no esta, pero antes lo hice). Los índices ya están alineados

```{r}
ph = read.table("fileListANIMAL_phylum.txt", header=FALSE)
head(ph)
ncol(ph)
...
ph = read.table("fileListANIMAL_phylum.txt", header = FALSE, sep = "")
species = ph$V1
phylum  = ph$V2
phylum_levels = levels(as.factor(phylum))

# Guardamos para colorear (vector)
phylums <- clase$phylum
```
...
```{r}
#devuelve las coordenadas principales
mdsVH <- cmdscale(as.dist(distVH), k = 2) # k=2 indica el número de dimensiones en las que queremos representar los datos despuees de aplicar el MDS.
mdsSP <- cmdscale(as.dist(distSP), k = 2)
mdsPM <- cmdscale(as.dist(distPM), k = 2)
```

```{r}
#asignamos colores
colors <- rainbow(length(phylum_levels))
col_vector <- colors[ match(phylums, phylum_levels) ]

#grafic
par(mfrow=c(1,3)) #tres gráficos colocados uno al lado del otro.

plot(mdsVH, col = col_vector, pch = 19,
     main = "MDS – Kernel VH")
legend("topright", legend = phylum_levels, col = colors, pch = 19)

plot(mdsSP, col = col_vector, pch = 19,
     main = "MDS – Kernel SP")
legend("topright", legend = phylum_levels, col = colors, pch = 19)

plot(mdsPM, col = col_vector, pch = 19,
     main = "MDS – Kernel PM")
legend("topright", legend = phylum_levels, col = colors, pch = 19)
```
