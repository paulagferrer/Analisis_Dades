...
```{r}
#bloque A
A = table(perfil, pam_Edu$clustering)

heatmap(A)
legend("right", legend = round(seq(min(A), max(A), length.out=5), 2),
       fill =heat.colors(5))

#bloque B
B = table(perfil, average_clusters)
heatmap(B)
legend("right", legend = round(seq(min(B), max(B), length.out=5), 2),
       fill =heat.colors(5))
```
