...
-   **Una matriz de gráficos de dispersión que incluya las correlaciones entre las variables: `Precisión`, `Robustez`, `Productividad`, `Ahorro_tiempo` y `Aceptacion_usuarios`por `Sector`**
    ...
    ggpairs(datosSelecion, 
            columns = which(sapply(datosSelecion, is.numeric)),
            aes(colors=Sector, alpha=0.3),
            upper = list( continuous = wrap( "cor", size=2, digits =2)))
    ```

    Como podemos ver, las correlaciones son muy significativas generalmente y en muchos casos valores proximos a 0.8, aa excepcion de Productividad\~Robustez que tiene significancia pero no muy alta. En cambio, algunas como Aceptacion\~Robustez tienen significancia en educacion pero muy baja correlacion.
...
-   **Una matriz de gráficos de dispersión que incluya las correlaciones entre las variables: `Riesgo_etico`, `Riesgo_legal` y `Coste` por `Sector`**
    ...
    datosSelecion2 = datos %>%
      select(Riesgo_etico, Riesgo_legal, Sector)

    ggpairs(datosSelecion2, 
            columns = which(sapply(datosSelecion2, is.numeric)),
            aes(colors=Sector, alpha=0.3),
            upper = list( continuous = wrap( "cor", size=3, digits =2)))
    ```

Como podemos ver las unicas variables que tenemos en este caso, estan altamente correlacionadas con mucha significacion.
...
#head(datosEdu)
pca_edu = prcomp(datosEdu, scale = TRUE)
summary(pca_edu)

#grafico codo
fviz_eig(pca_edu, addlabels = TRUE, ylim=c(0,100))
# cogemos las dos primeras dimesiones para obtener un 81% de la informacion

#grafico1
fviz_pca_var(pca_edu, col.var = "contrib",
             gradient.cols = c("#00AFBB", "#E7B800", "#FC4E07"),
             repel = TRUE) 

#grafico2
fviz_pca_biplot(pca_edu, repel = TRUE,
                col.var = "darkblue", # color para las variables
                col.ind = "grey"  # color para las observaciones
                )
```

Interpretación:

Grafico2 = Todas las variables estan bien representadas y las que estan más juntas son las que estan más correlacionadas. Variables como Riesgo legal y etico estan muy correlacionadas y mejor representadas por la primera componente. En cambio, otras como Aceptacion, ahorro, robustez, no estan tan correlacionadas y tienen más peso sus loadings en la segunda componentes.

Grafico1 = En general todas las variables contribuyen bien a las componentes principales y en cambio productividad no contribuye tanto como las demás, si tuvieramos que hacer una agrupacion por productividad no podriamos utilizar esta representacion, ya que no es muy significativa.
...
#head(datosEdu)
pca_sa = prcomp(datosSa, scale = TRUE)
summary(pca_sa)

#grafico codo
fviz_eig(pca_sa, addlabels = TRUE, ylim=c(0,100))
# cogemos las dos primeras dimesiones para obtener un 79% aprx de la informacion

#grafico1
fviz_pca_var(pca_sa, col.var = "contrib",
             gradient.cols = c("#00AFBB", "#E7B800", "#FC4E07"),
             repel = TRUE) 

#grafico2
fviz_pca_biplot(pca_sa, repel = TRUE,
                col.var = "darkblue", # color para las variables
                col.ind = "grey"  # color para las observaciones
                )
```
...
Grafico2 = como comentamos en el caso de educacion nos pasa lo mismo con las variables.

Grafico1 = En general todas las variables contribuyen bien a las componentes principales y en cambio coste no contribuye tanto como las demás, si tuvieramos que hacer una agrupacion por coste no podriamos utilizar esta representacion, ya que no es muy significativa.
...
Respuesta: Aplicamos un k-medoids y utilizamos la funcion PAM.

Seleccionamos las variables numericas que nos interesan, filtradas por Educacion, las escalamos y el primer grafico que nos sale nos sirve para seleccionar el numero de clusters que podriamos hacer, en el codigo se han seleccionado 4, aunque a partir de 3 clusters la reducción en la suma total de cuadrados internos parece estabilizarse, indicando que k=3 es una buena opción. Sin embargo, si observamos perfiles, solo hay 4 componentes diferentes.
...
Respuesta: Se calcula la matriz de distancias euclídeas sobre datos estandarizados, se aplica un clustering jerárquico aglomerativo con enlace average, se evalúa su calidad mediante la correlación, se visualiza el dendrograma, se fija una partición en 4 clústeres y finalmente se interpretan los grupos en función de una variable externa de perfil.

Escogemos el bloque B, porque agrupa mejor los perfiles en cada cluster.
