...
El vector de observaciones $Y \in \mathbb{R}^n$ viene relacionada por la matriz $X$ de datos y los parámetros $\beta$ según la relación
...
\begin{cases}
\beta = (X^tX)^{-1}X^tY \\
\sigma^2 = \dfrac{(Y-X\beta)^t(Y-X\beta)}{n}
\end{cases}$$

Por lo que necesitamos que $X^t X$ sea invertible o, equivalentemente, que el rango de $X$ sea máximo.
Lo metemos en R:
...
```
El score es el gradiente de la log-verosimilitud, indica la dirección en que habría que modificar los parámetros para aumentar la verosimilitud. En el máximo de verosimilitud el score se anula, es decir, el modelo ajusta a los datos observados. Ahora bien, este máximo no és único ya que el rango de $X$ no es máximo, ya que de hecho $X_2 = 0.5 \cdot X_1 + 50$.

Ahora bien, debido a esta relación lineal podemos despreciar una de ellas imponiendo que, por ejemplo, $\beta_2 = 0$, ya que el modelo lineal se puede reescribir como $Y = \beta_0 + \beta_1 X_1 + \beta_2 X_2 = \beta_0 + \beta_2 \cdot 50 + (\beta_1 + 0.5 \cdot \beta_2) X_1$, es decir, que solo dependa linealmente de $X_1$.

Rehaciendo los cálculos obviando $X_2$,

```{r}
X_red <- X[,-which(colnames(X) == "X2")]
...
epsilon
```
Como observamos los datos se ajustan bastante bien al modelo lineal, con la varianza del error bastante pequeña respecto a los valores.
